...
public class TransClass {
	...
	private static Logger logger = Logger
			.getLogger("de.unifr.acp.trafo.TransClass");
	...
	private void doTraverse(CtClass target, Queue<CtClass> pending,
		...
		for (CtClass type : referredTypes) {
			...
			if (typeName.matches(filterVisitRegex)) {
				logger.fine("Filtering class " + typeName);
				continue;
			}
			enter(type, pending, visited);
		}
...
//							+ ".class.getClassLoader();");
			...
			if (methodOrCtor instanceof CtConstructor) {
				...
				if (!constructor.isClassInitializer()) {
					logger.fine("Insert header in " + methodName);
					((CtConstructor) methodOrCtor).insertBeforeBody(header);

					logger.fine("Insert footer in " + methodName);
	...
	protected static String createTraverseStaticsBody(CtClass target)
		...
		for (CtField f : membersMatchingAllModifers(
			...
			sb.append("{");
			sb.append("  try {");
			sb.append("    System.out.print(\"\");"); // avoid empty try block
	...
	protected static String createStaticsOnClassLoadTraversalBody(CtClass target) {
		...
		// using reflection (Class.forName) for obtaining a Class object
		// instead of ClassName.class to avoid invalid identifier problems
		sb.append("  " + GLOBAL_CLASS_NAME + ".traverseStaticsOnClassLoad("
