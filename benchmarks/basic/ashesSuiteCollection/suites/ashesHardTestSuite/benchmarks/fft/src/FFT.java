...
public class FFT {
  ...
  public static final double num_flops(int N)
  {
     ...
     double out = (5.0*Nd-2)*logN + 2*(Nd+1);
	 return out;
   }
  ...
  public static double test(double data[]){
    ...
    for(int i=0; i<nd; i++) {
      ...
      diff += d*d; 	}
    double out = Math.sqrt(diff/nd); 
	return out;   }

  /** Make a random array of n (complex) elements. */
  public static double[] makeRandom(int n, Random R){
    int nd = 2*n;
...
if ( N%2 != 0) 	{
	  ...
	  data = makeRandom(n,R);	  double out = test(data);
      System.out.println("n="+n+" => RMS Error=" + out);     }
while (N%2 == 0) 	{	  int n=N;	  double[] data = new double[2*n];
	  data = makeRandom(n,R);	  double out = test(data);
      System.out.println("n="+n+" => RMS Error="+ out );      N=n/2; 		}
long T2=System.currentTimeMillis();
//long T = T2 - T1;
//System.out.println("The running time of this system is :"+T+"*10^-3 seconds. "); 
 ...
 //System.out.println("os.version:" + System.getProperty("os.vesion")); 
  }}
